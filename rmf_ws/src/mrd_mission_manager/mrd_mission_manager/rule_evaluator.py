...
def evaluate_rules(state: MissionState):
    ...
    transfer = TransferController(state.transfer)
...
def _grant_transfer_entry(state: MissionState, transfer: TransferController):
    ...
    return [DispatchTask(robot_id, package_id, TaskSegment.STAGING_TO_TRANSFER)]


def _continue_downstream_delivery(state: MissionState):
    robot = state.robots[DOWNSTREAM_ROBOT]
...
def _start_downstream_package(state: MissionState, transfer: TransferController):
    ...
    if package.status != PackageStatus.AT_TRANSFER:
        return []
    if not transfer.can_robot_enter(DOWNSTREAM_ROBOT, package_id):
        return []

    transfer.occupy_transfer(DOWNSTREAM_ROBOT)
    return [DispatchTask(DOWNSTREAM_ROBOT, package_id, TaskSegment.HOME_TO_TRANSFER)]


def _start_upstream_package(state: MissionState):
    robot = state.robots[UPSTREAM_ROBOT]
